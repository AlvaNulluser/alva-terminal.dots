...
const float OPACITY = 0.6;          // Punto medio: Más opaco que Gentleman (0.4) pero con algo de transparencia
const float DURATION = 0.2;         // Punto medio: Estela de 200ms (Gentleman: 100ms, Glow: 300ms)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Add very subtle glow around cursor (middle ground parameters)
    float glowDist = sdfCurrentCursor + 0.005;
    newColor = mix(newColor, trail * 0.3, smoothstep(0.015, 0.0, glowDist));
